...
describe('Ventas Endpoints', () => {
  ...
  describe('GET /', () => {
    ...
    test('should return list of sales', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ 
        data: expect.arrayContaining([
          expect.objectContaining({ id: 'v-1', monto_total: 1000 })
        ])
      }));
    });
    ...
    test('should deny access if no valid scope', async () => {
        const req = { 
          ...
          expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: [{ message: 'Insufficient scopes', code: 'FORBIDDEN' }] }));
    });
    ...
    test('should return 500 if user context not found (read:ventas:own)', async () => {
        ...
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: [{ message: 'User context not found' }] }));
    });
    ...
    test('should handle errors and return 500', async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(500);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: [{ message: 'Database error' }] }));
    });
  });
  ...
  describe('POST /', () => {
    ...
    test('should create a sale and return 201', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ id: 'v-new' }) }));
    });
    ...
    test('should fail if client does not exist', async () => {
        ...
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: [{ message: 'Cliente not found', code: 'NOT_FOUND' }] }));
    });
    ...
    test('should fail if lote does not exist', async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(404);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: [{ message: 'Lote not found', code: 'NOT_FOUND' }] }));
    });
    ...
    test('should fail if lote is not available', async () => {
        ...
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: [{ message: 'Lote not available', code: 'LOTE_NOT_AVAILABLE' }] }));
    });
    ...
    test('should fail if enganche exceeds price', async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: [{ message: 'Enganche exceeds price', code: 'INVALID_AMOUNT' }] }));
    });
    ...
    test('should generate amortizations and commissions for financed sale', async () => {
        ...
        itemsServiceInstance.readOne
          ...
          .mockResolvedValueOnce({ id: '123e4567-e89b-12d3-a456-426614174001', estatus: 'disponible', precio_lista: 100000 }) // Lote
          .mockResolvedValueOnce({ id: 'v-new', monto_total: 100000 }) // Lectura final venta
          .mockResolvedValueOnce({ id: '123e4567-e89b-12d3-a456-426614174002', comision_porcentaje: 5 }); // Vendedor info for commissions
    ...
    test('should rollback transaction if commission generation fails', async () => {
        const req = {
          ...
          itemsServiceInstance.readOne
            ...
            .mockResolvedValueOnce({ id: '123e4567-e89b-12d3-a456-426614174002', comision_porcentaje: 5 }); // Vendedor
          ...
          expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: [{ message: 'Commission Error' }] }));
    });
    ...
    test('should calculate correct monthly payment with 0% interest', async () => {
        ...
        itemsServiceInstance.readOne
          ...
          .mockResolvedValueOnce({ id: '123e4567-e89b-12d3-a456-426614174001', estatus: 'disponible', precio_lista: 100000 })
  ...
  describe('Rate Limiter Middleware', () => {
      ...
      test('should block requests over limit', () => {
        ...
        expect(resBlocked.json).toHaveBeenCalledWith(expect.objectContaining({ errors: expect.arrayContaining([expect.objectContaining({ code: 'RATE_LIMIT_EXCEEDED' })]) }));
