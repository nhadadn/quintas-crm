...
describe('Comisiones Endpoint', () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        errors: expect.arrayContaining([
          expect.objectContaining({ message: expect.stringContaining('Falta parámetro') }),
        ]),
      })
    );
  });
  ...
  test('should handle missing seller', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        errors: expect.arrayContaining([
          expect.objectContaining({
            message: expect.stringContaining('no tiene vendedor asignado'),
          }),
        ]),
      })
    );
  ...
  test('should return 404 if venta not found', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        errors: expect.arrayContaining([
          expect.objectContaining({ message: expect.stringContaining('Venta v-99 no encontrada') }),
        ]),
      })
    );
  });
  ...
  test('should return 404 if vendedor not found', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        errors: expect.arrayContaining([
          expect.objectContaining({
            message: expect.stringContaining('Vendedor vend-99 no encontrado'),
          }),
        ]),
      })
    );
