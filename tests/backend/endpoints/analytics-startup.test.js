...
describe('Analytics Custom Extension - Startup Logic', () => {
  ...
  beforeEach(() => {
    ...
    // Mock Services
    rolesServiceMock = {
      readByQuery: jest.fn().mockResolvedValue([{ id: 'role-1', name: 'Vendedor' }]),
    };

    policiesServiceMock = {
      readByQuery: jest.fn().mockResolvedValue([{ id: 'policy-public', name: 'Public' }]),
    };
  ...
  test('should handle Policy-based permissions (Directus 10.10+)', async () => {
    ...
    expect(policiesServiceMock.readByQuery).toHaveBeenCalledWith(
      expect.objectContaining({
