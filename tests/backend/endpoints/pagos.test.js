...
import { createPaymentIntent, constructEvent, createOrRetrieveCustomer } from '../../../extensions/endpoint-pagos/src/stripe-service.js';
...
describe('Pagos API Extension', () => {
  ...
  describe('POST /', () => {
    ...
    test('should register manual payment successfully', async () => {
      ...
      itemsServiceInstance.readOne.mockResolvedValue({ id: 'pago-1', monto: 100, estatus: 'pagado' });
      ...
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ 
        ...
        meta: expect.objectContaining({ message: 'Pago registrado exitosamente' })
      }));
    });
    ...
    test('should return 400 if no pending payments found', async () => {
        ...
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
            errors: expect.arrayContaining([
                expect.objectContaining({ message: 'No se encontraron pagos pendientes para esta venta' })
            ])
        }));
    });
    ...
    test('should return 400 if payment amount exceeds pending balance', async () => {
        ...
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
            errors: expect.arrayContaining([
                expect.objectContaining({ message: expect.stringContaining('excede el saldo pendiente') })
            ])
        }));
    });
    ...
    test('should not mark as paid if amount is partial', async () => {
        ...
        const { ItemsService } = mockContext.services;
        const itemsServiceInstance = new ItemsService();
        itemsServiceInstance.readOne.mockResolvedValue({ id: 'pago-1', monto: 100, estatus: 'pendiente' });
    ...
    test('should return 400 if pago_id not found', async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: [{ message: 'Pago no encontrado' }] }));
    });
    ...
    test('should return 400 if pago_id does not match venta_id', async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: [{ message: 'El pago no pertenece a la venta especificada' }] }));
    });
    ...
    test('should fail if monto is negative', async () => {
        ...
        await postHandler(req, res);
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: expect.arrayContaining([expect.objectContaining({ message: expect.stringContaining('monto') })]) }));
    });
  ...
  describe('POST /create-payment-intent', () => {
    ...
    test('should create payment intent', async () => {
      ...
      itemsServiceInstance.readOne.mockResolvedValue({ id: 'c-1', stripe_customer_id: 'cus_123', email: 'test@example.com', nombre: 'Test' });

      createPaymentIntent.mockResolvedValue({ client_secret: 'secret_123' });

      await createIntentHandler(req, res);

      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ clientSecret: 'secret_123' }));
    });
    ...
    test('should fail if payment already processed', async () => {
        ...
        itemsServiceInstance.readByQuery.mockResolvedValue([{ 
            id: 'pago-1', estatus: 'pagado', venta_id: { id: 'v-1', cliente_id: 'c-1' } 
        }]);
    ...
    test('should create new stripe customer if not exists', async () => {
        ...
        itemsServiceInstance.readByQuery.mockResolvedValue([
            { id: 'pago-1', monto: 100, estatus: 'pendiente', venta_id: { id: 'v-1', cliente_id: 'c-1' }, numero_pago: 1 }
        ]);
        
        // Mock client WITHOUT stripe_customer_id
        itemsServiceInstance.readOne.mockResolvedValue({ id: 'c-1', email: 'new@example.com', nombre: 'New Client' });
        ...
        expect(itemsServiceInstance.updateOne).toHaveBeenCalledWith('c-1', { stripe_customer_id: 'new_cus_123' });
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ clientSecret: 'secret_123' }));
    });
    ...
    test('should fail if amount to pay is zero or negative', async () => {
        ...
        itemsServiceInstance.readByQuery.mockResolvedValue([
            { id: 'pago-1', monto: 100, monto_pagado: 100, estatus: 'pendiente', venta_id: { id: 'v-1', cliente_id: 'c-1' }, numero_pago: 1 }
        ]);
        ...
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: expect.arrayContaining([expect.objectContaining({ message: expect.stringContaining('mayor a 0') })]) }));
    });

    test('should handle NotFoundException', async () => {
        if (!createIntentHandler) return;
        const req = { body: { cliente_id: 'c-1', venta_id: 'v-99', numero_pago: 1 }, accountability: { user: 'admin' }, ip: '127.0.0.1', connection: { remoteAddress: '127.0.0.1' } };
        const res = mockRes();
        const { ItemsService } = mockContext.services;
        const itemsServiceInstance = new ItemsService();
    ...
    test('should handle ForbiddenException', async () => {
        if (!createIntentHandler) return;
        const req = { body: { cliente_id: 'c-2', venta_id: 'v-1', numero_pago: 1 }, accountability: { user: 'admin' }, ip: '127.0.0.1', connection: { remoteAddress: '127.0.0.1' } };
        const res = mockRes();
        const { ItemsService } = mockContext.services;
        const itemsServiceInstance = new ItemsService();
        itemsServiceInstance.readByQuery.mockResolvedValue([{ id: 'pago-1', venta_id: { id: 'v-1', cliente_id: 'c-1' }, numero_pago: 1 }]); // Belongs to c-1
    ...
    test('should return 500 on generic error', async () => {
        ...
        itemsServiceInstance.readByQuery.mockResolvedValue([
            { id: 1, venta_id: { id: 1, cliente_id: 1, lote_id: 1 }, monto: 1000, estatus: 'pendiente', numero_pago: 1 }
        ]);
        itemsServiceInstance.readOne.mockResolvedValue({ id: 1, stripe_customer_id: 'cus_123' });
        
        // Mock createPaymentIntent failure
        const { createPaymentIntent } = require('../../../extensions/endpoint-pagos/src/stripe-service.js');
        createPaymentIntent.mockRejectedValue(new Error('Stripe API Error'));
  
        await createIntentHandler(req, res);
  
        expect(res.status).toHaveBeenCalledWith(500);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: [{ message: 'Stripe API Error' }] }));
    });
    ...
    test('should handle payment_intent creation with pago_id', async () => {
        ...
        itemsServiceInstance.readOne
            .mockResolvedValueOnce({ id: 1, venta_id: { id: 1, cliente_id: 1, lote_id: 1 }, monto: 1000, estatus: 'pendiente', numero_pago: 1 }) // Pago
            .mockResolvedValueOnce({ id: 1, stripe_customer_id: 'cus_123' }); // Cliente
        ...
        expect(itemsServiceInstance.readOne).toHaveBeenCalledWith(1, expect.any(Object));
    });

    test('should return 429 if rate limit exceeded', async () => {
        const req = {
            body: { cliente_id: 1, venta_id: 1, numero_pago: 1 },
            accountability: { user: 'admin' },
            ip: '1.2.3.4', connection: { remoteAddress: '1.2.3.4' }
        };
        const res = mockRes();
        const { ItemsService } = mockContext.services;
        const itemsServiceInstance = new ItemsService();
        itemsServiceInstance.readByQuery.mockResolvedValue([{ id: 1 }]);
        itemsServiceInstance.readOne.mockResolvedValue({ id: 1, stripe_customer_id: 'cus' });
        
        const { createPaymentIntent } = require('../../../extensions/endpoint-pagos/src/stripe-service.js');
        createPaymentIntent.mockResolvedValue({ id: 'pi', client_secret: 'sec' });

        // Call 6 times (Limit is 5)
        for (let i = 0; i < 5; i++) {
            await createIntentHandler(req, mockRes());
        }
        await createIntentHandler(req, res);

        expect(res.status).toHaveBeenCalledWith(429);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: [expect.objectContaining({ code: 'RATE_LIMIT_EXCEEDED' })] }));
    });
  ...
  describe('POST /webhook', () => {
      ...
      test('should register POST /webhook route', () => {
          expect(router.post).toHaveBeenCalledWith('/webhook', expect.any(Function));
      });
      ...
      test('should handle payment_intent.succeeded', async () => {
          ...
          itemsServiceInstance.readByQuery.mockResolvedValue([{ id: 'pago-1', estatus: 'pendiente' }]);
  
          await webhookHandler(req, res);
  
          expect(itemsServiceInstance.updateOne).toHaveBeenCalledWith('pago-1', expect.objectContaining({
              estatus: 'pagado',
              stripe_last4: '4242'
          }));
      });
    ...
    test('should ignore already paid payment in succeeded event', async () => {
          ...
          itemsServiceInstance.readByQuery.mockResolvedValue([{ id: 'pago-1', estatus: 'pagado' }]);
      ...
      test('should handle payment_intent.payment_failed', async () => {
          ...
          itemsServiceInstance.readByQuery.mockResolvedValue([{ id: 'pago-1', estatus: 'pendiente', notas: 'Old notes' }]);
      ...
      test('should warn if payment not found for successful intent', async () => {
          ...
          itemsServiceInstance.readByQuery.mockResolvedValue([]); // Empty
      ...
      test('should handle invoice.payment_succeeded', async () => {
          ...
          expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Invoice payment succeeded: inv_1'));
      ...
      test('should skip signature verification if secret is missing', async () => {
          ...
          const req = { body: { type: 'test_event' }, headers: {} };
          const res = mockRes();
      ...
      test('should handle customer.subscription events', async () => {
          ...
          expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Subscription event customer.subscription.created: sub_123'));
          consoleSpy.mockRestore();
      });
      ...
      test('should handle string payload (skip stringify)', async () => {
          ...
          const res = mockRes();
          constructEvent.mockReturnValue({ type: 'unknown', data: {} });
          
          await webhookHandler(req, res);
          
          expect(constructEvent).toHaveBeenCalledWith('raw_string_payload', 'valid', expect.any(String));
      });
  ...
  describe('PATCH /:id', () => {
    ...
    test('should update payment successfully', async () => {
        ...
        expect(itemsServiceInstance.updateOne).toHaveBeenCalledWith('pago-1', { monto: 200, notas: 'Updated' });
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ message: 'Pago actualizado' }) }));
    });
    ...
    test('should forbid update if payment is paid', async () => {
        ...
        expect(res.status).toHaveBeenCalledWith(403);
        expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: expect.arrayContaining([expect.objectContaining({ message: expect.stringContaining('No se puede editar') })]) }));
    });
    ...
    test('should return 400 if no valid fields provided', async () => {
        ...
        // It throws InvalidPayloadException which is not caught in PATCH, so falls to 500
        expect(res.status).toHaveBeenCalledWith(500); 
    });
  ...
  describe('DELETE /:id', () => {
      ...
      test('should return 403 Forbidden', async () => {
          ...
          expect(res.status).toHaveBeenCalledWith(403);
          expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ errors: expect.arrayContaining([expect.objectContaining({ code: 'FORBIDDEN' })]) }));
      });
  });
});
