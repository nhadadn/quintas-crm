...
class MockItemsService {
  ...
  async readOne(id) {
    if (this.collection === 'vendedores') {
      if (id === 'vend-1')
        return { id: 'vend-1', comision_porcentaje: 5, comision_esquema: 'porcentaje' };
      if (id === 'vend-mixed')
        return { id: 'vend-mixed', comision_porcentaje: 3, comision_esquema: 'mixto' }; // 3% + fixed (assumed fixed comes from somewhere else or config, but here we test logic)
...
async function runTests() {
  ...
  try {
    ...
    MockItemsService.prototype.readOne = async function (id) {
      ...
      if (id === 'venta-pendiente')
        return { estatus: 'apartado', monto_financiado: 50000, plazo_meses: 24 };
      if (id === 'vend-1')
        return { id: 'vend-1', comision_porcentaje: 5, comision_esquema: 'porcentaje' };
      return null;
    };
