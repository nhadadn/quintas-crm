...
async function fixPermissions() {
    ...
    try {
        ...
        const [adminRoles] = await connection.execute('SELECT id FROM directus_roles WHERE name = "Administrator"');
        let adminRoleId;
        ...
        const [adminPolicies] = await connection.execute('SELECT id, name FROM directus_policies WHERE admin_access = 1');
        let adminPolicyId;
        if (adminPolicies.length > 0) {
            ...
            console.log('⚠️ No policy with admin_access=1 found. Checking for "Administrator" policy name...');
            const [namedPolicies] = await connection.execute('SELECT id FROM directus_policies WHERE name = "Administrator"');
            if (namedPolicies.length > 0) {
                adminPolicyId = namedPolicies[0].id;
                console.log(`Found Policy by name "Administrator": ${adminPolicyId}. Updating admin_access...`);
                await connection.execute('UPDATE directus_policies SET admin_access = 1, app_access = 1 WHERE id = ?', [adminPolicyId]);
            } else {
        ...
        // Verify Role Exists
        const [vRoles] = await connection.execute('SELECT id FROM directus_roles WHERE id = ?', [VENDEDOR_ROLE_ID]);
        if (vRoles.length === 0) {
        ...
        const [vPolicies] = await connection.execute('SELECT id FROM directus_policies WHERE id = ?', [VENDEDOR_POLICY_ID]);
        if (vPolicies.length === 0) {
            ...
            // Ensure app_access is 1
             await connection.execute('UPDATE directus_policies SET app_access = 1 WHERE id = ?', [VENDEDOR_POLICY_ID]);
        ...
        if (vAccess.length === 0) {
            ...
            await connection.execute(
                'INSERT INTO directus_access (id, role, policy) VALUES (?, ?, ?)',
                [newId, VENDEDOR_ROLE_ID, VENDEDOR_POLICY_ID]
            );
        ...
        const [users] = await connection.execute('SELECT id, role FROM directus_users WHERE email = ?', [USER_EMAIL]);
        if (users.length > 0) {
            const user = users[0];
            if (user.role !== VENDEDOR_ROLE_ID) {
                console.log(`Updating user ${USER_EMAIL} role from ${user.role} to ${VENDEDOR_ROLE_ID}`);
                await connection.execute('UPDATE directus_users SET role = ? WHERE id = ?', [VENDEDOR_ROLE_ID, user.id]);
