...
async function fixClientesSchema() {
    ...
    try {
        ...
        const [columns] = await connection.execute("SHOW COLUMNS FROM clientes WHERE Field = 'user_created'");
        ...
        const [updatedCols] = await connection.execute("SHOW COLUMNS FROM clientes WHERE Field = 'user_updated'");
        ...
        const [fields] = await connection.execute("SELECT id FROM directus_fields WHERE collection = 'clientes' AND field = 'user_created'");
        if (fields.length === 0) {
        ...
        const [fieldsUp] = await connection.execute("SELECT id FROM directus_fields WHERE collection = 'clientes' AND field = 'user_updated'");
