...
async function checkVendedorUser() {
  ...
  const searchRes = await fetch(
    `${DIRECTUS_URL}/users?filter[_or][0][first_name][_contains]=Erick&filter[_or][1][email][_contains]=Erick&fields=*,role.*`,
    {
  ...
  if (searchData.data && searchData.data.length > 0) {
    ...
    if (user.status === 'active' && user.role && user.role.name === 'Vendedor') {
      ...
      if (!user.role || user.role.name !== 'Vendedor')
        console.log(
          `   - User does NOT have Vendedor role (Role: ${user.role ? user.role.name : 'None'})`
        );
    }
  } else {
    ...
    console.log(
      '\nExisting Users:',
      allData.data.map((u) => `${u.first_name} ${u.last_name} (${u.email})`).join(', ')
    );
  }
}
