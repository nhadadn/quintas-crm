...
async function verifyPermissions() {
  ...
  console.log(`   [Ventas: Read] ${ventasRes.ok ? '✅ Allowed' : '❌ Denied'} - Found: ${ventasData.data ? ventasData.data.length : 0}`);
  ...
  console.log(`   [Clientes: Read] ${clientesRes.ok ? '✅ Allowed' : '❌ Denied'} - Found: ${clientesData.data ? clientesData.data.length : 0}`);

  // E. Read Users (Should see self)
  const usersRes = await fetch(`${DIRECTUS_URL}/users/${userId}`, { headers: userHeaders });
  console.log(`   [Users: Read Self] ${usersRes.ok ? '✅ Allowed' : '❌ Denied'} (${usersRes.status})`);
  
  const otherUserRes = await fetch(`${DIRECTUS_URL}/users?limit=1&filter[id][_neq]=${userId}`, { headers: userHeaders });
  ...
  console.log(`   [Users: Read All] Found: ${otherUserData.data ? otherUserData.data.length : 0} (Should be 0 or contain only self)`);
