...
async function setup() {
  ...
  let roleId;
  try {
    const rolesRes = await fetch(`${DIRECTUS_URL}/roles?filter[name][_eq]=Vendedor`, { headers: authHeaders });
  ...
  const permissions = [
    ...
    { collection: 'ventas', action: 'read', fields: '*', permissions: { vendedor_id: { _eq: '$CURRENT_USER' } } },
    ...
    { collection: 'clientes', action: 'read', fields: '*', permissions: { _or: [{ user_created: { _eq: '$CURRENT_USER' } }, { user_id: { _eq: '$CURRENT_USER' } }] } }, // Try both
    { collection: 'clientes', action: 'update', fields: '*', permissions: { _or: [{ user_created: { _eq: '$CURRENT_USER' } }, { user_id: { _eq: '$CURRENT_USER' } }] } },

    // Pagos: Read (Mine via Venta)
    { collection: 'pagos', action: 'read', fields: '*', permissions: { venta_id: { vendedor_id: { _eq: '$CURRENT_USER' } } } },

    // Comisiones: Read (Mine)
    { collection: 'comisiones', action: 'read', fields: '*', permissions: { vendedor_id: { _eq: '$CURRENT_USER' } } },

    // Usuarios: Read/Update (Self)
    { collection: 'directus_users', action: 'read', fields: '*', permissions: { id: { _eq: '$CURRENT_USER' } } },
    { collection: 'directus_users', action: 'update', fields: '*', permissions: { id: { _eq: '$CURRENT_USER' } } },
  ];
  ...
  try {
    ...
    const policiesRes = await fetch(`${DIRECTUS_URL}/policies?filter[name][_eq]=Vendedor Policy`, { headers: authHeaders });
    ...
    const permRes = await fetch(`${DIRECTUS_URL}/permissions?filter[policy][_eq]=${policyId}&limit=-1`, { headers: authHeaders });
