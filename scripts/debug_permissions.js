...
async function debugPermissions() {
      ...
      for (const policyId of role.policies) {
          ...
          if (permData.data) {
            permData.data.forEach(p => {
                console.log(`   - Collection: ${p.collection} | Action: ${p.action} | Fields: ${p.fields} | Rule: ${JSON.stringify(p.permissions)}`);
            });
          }
      }
  }
  ...
  const rolePermRes = await fetch(`${DIRECTUS_URL}/permissions?filter[role][_eq]=${roleId}`, { headers });
  if (rolePermRes.ok) {
