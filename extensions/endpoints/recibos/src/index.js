...
export default (router, { services, exceptions }) => {
	...
	router.get('/:id/generar', async (req, res) => {
		try {
			...
			// Detalles Pago
			const startY = 170;
			doc.fillColor('black').fontSize(10).font('Helvetica-Bold').text('DETALLES DEL PAGO', 50, startY);
			doc.font('Helvetica').text(`Folio: ${pago.numero_pago}`, 50, startY + 20);
			doc.text(`Fecha: ${pago.fecha_pago ? new Date(pago.fecha_pago).toLocaleDateString() : 'Pendiente'}`, 50, startY + 35);
			doc.text(`Monto: $${parseFloat(pago.monto_pagado || 0).toLocaleString('es-MX', { minimumFractionDigits: 2 })}`, 50, startY + 50);
			...
			doc.font('Helvetica');
			if (cliente) {
				doc.text(`${cliente.nombre} ${cliente.apellido_paterno} ${cliente.apellido_materno || ''}`, 300, startY + 20);
            ...
            doc.fillColor('#000000').fontSize(14).font('Helvetica-Bold')
               .text(`TOTAL PAGADO: $${parseFloat(pago.monto_pagado || 0).toLocaleString('es-MX', { minimumFractionDigits: 2 })} MXN`, 0, boxY + 15, { align: 'center', width: 600 });

			// Footer
			doc.fontSize(8).fillColor('grey').text('Este recibo es un comprobante digital generado por el sistema Quintas CRM.', 50, 700, { align: 'center' });
