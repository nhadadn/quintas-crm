	...
	router.post('/', async (req, res) => {
		try {
			...
			if (error.code === 'ER_DUP_ENTRY') {
				return res.status(409).json({ errors: [{ message: "Registro duplicado (Email o RFC ya existe)", code: 'DUPLICATE_ENTRY' }] });
			}
	...
	router.patch('/:id', async (req, res) => {
		try {
			...
			if (payload.email) {
				...
				if (existing.length > 0) throw new InvalidPayloadException(`El email ${payload.email} ya está usado por otro cliente.`);
			}
			...
			if (payload.rfc) {
				...
				if (existing.length > 0) throw new InvalidPayloadException(`El RFC ${payload.rfc} ya está usado por otro cliente.`);
			}
			...
			if (error.code === 'ER_DUP_ENTRY') return res.status(409).json({ errors: [{ message: "Registro duplicado", code: 'DUPLICATE_ENTRY' }] });
