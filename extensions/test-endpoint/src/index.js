export default (router, { services, database }) => {
  ...
  router.post('/create-test-user', async (req, res) => {
    try {
      ...
      if (existing && existing.length > 0) {
        ...
        const client = (
          await itemsService.readByQuery({ filter: { user_id: { _eq: userId } }, limit: 1 })
        )[0];
  ...
  router.get('/inspect-data', async (req, res) => {
    try {
      ...
      const clientes = await clientesService.readByQuery({
        fields: ['*'],
        limit: 10,
        filter: { email: { _eq: 'cliente.prueba@quintas.com' } },
      });
  ...
  router.get('/check-relations', async (req, res) => {
    try {
      const relations = await database('directus_relations').where({
