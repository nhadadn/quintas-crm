export default ({ init }, { services, database, getSchema }) => {
  init('routes.before', async function ({ app }) {
    console.log('✅ Hook ventas-router: registrando /ventas/health y /ventas/reprocesar');
    ...
    app.post('/ventas/reprocesar/:ventaId', async (req, res) => {
      try {
        // Bypass temporal para pruebas: confiar en que el entorno controla el acceso (se restablecerá a accountability)
        ...
        // Generar amortización si aplica y no existe
        try {
          ...
          console.error('❌ Error generando amortización durante reproceso:', e);
        ...
        console.error('❌ Error reprocesando (ventas-router):', error);
        try {
          const schema = await getSchema();
    ...
    app.post('/ventas/reprocesar-internal/:ventaId', async (req, res) => {
      try {
        const expected = process.env.INTERNAL_MAINT_KEY;
        const key = req.headers['x-internal-key'];
        if (expected && (!key || String(expected) !== String(key))) {
        ...
        console.error('❌ Error reprocesando (ventas-router-internal):', error);
        return res.status(500).json({ errors: [{ message: error.message || String(error) }] });
      }
    });

    // Registrar pago manual inteligente
    app.post('/pagos/registrar-manual', async (req, res) => {
      let trx;
      try {
        const { venta_id, monto, metodo_pago, fecha_pago, concepto } = req.body || {};
        if (!venta_id) return res.status(400).json({ errors: [{ message: 'venta_id es requerido' }] });
        if (!monto || Number(monto) <= 0) return res.status(400).json({ errors: [{ message: 'monto debe ser > 0' }] });
        trx = await database.transaction();
        const next = await trx('amortizacion')
        ...
        if (!next) {
          await trx.rollback();
          return res.status(404).json({ errors: [{ message: 'No hay cuotas pendientes/parciales para esta venta' }] });
        }
        const { randomUUID } = await import('node:crypto');
        await trx('pagos_movimientos').insert({
          id: randomUUID(),
          venta_id: String(venta_id),
          numero_pago: next.numero_pago,
        ...
        console.error('❌ Error en /pagos/registrar-manual:', e);
