...
describe('Directus API Coverage', () => {
  ...
  describe('handleAxiosError Edge Cases', () => {
    it('should use default message for 403 if no details provided', () => {
      ...
      expect(() => handleAxiosError(error, 'test')).toThrow(
        'Acceso denegado a recurso de Directus',
      );
    ...
    it('should use default message for 400 if errors array is empty', () => {
      ...
      expect(() => handleAxiosError(error, 'test')).toThrow(
        'Parámetros inválidos para la petición a Directus',
      );
    });
  });
  ...
  describe('convertToGeoJSON', () => {
    ...
    it('should handle invalid JSON in geometria field gracefully', () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining('Error parseando geometría'),
        expect.any(Error),
      );
      consoleWarnSpy.mockRestore();
    });
    ...
    it('should handle error in UTM conversion gracefully', () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Error convirtiendo coordenadas UTM'),
        expect.any(Error),
      );
  ...
  describe('fetchLotesAsGeoJSON', () => {
    ...
    it('should fallback to public endpoint on 401/403 with token', async () => {
      ...
      expect(axios.get).toHaveBeenCalledWith(
        expect.stringContaining('/mapa-lotes'),
        expect.any(Object),
      );
      expect(result).toEqual(mockGeoJSON);
    });
    ...
    it('should apply all client-side filters', async () => {
      ...
      mockClient.get.mockImplementation(() =>
        Promise.resolve({ data: JSON.parse(JSON.stringify(mockGeoJSON)) }),
      );
  ...
  describe('Interceptors', () => {
    ...
    it('should log request details', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('[Directus Request] GET /test'),
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('Auth: Bearer long...s123'),
      ); // Masked correctly
    ...
    it('should log response details', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('[Directus Response] 200 /test'),
      );
    ...
    it('should NOT retry 401 if token is same', async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining('El token renovado es igual al anterior'),
      );
    ...
    it('should handle session refresh failure', async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining('No se pudo recuperar una sesión válida'),
      );
      consoleWarnSpy.mockRestore();
    });
    ...
    it('should handle session refresh error exception', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('Error al intentar renovar la sesión'),
        expect.any(Error),
      );
      consoleErrorSpy.mockRestore();
    });
    ...
    it('should handle config error', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringContaining('[Directus Config Error]'),
        'Config Error',
      );
      consoleErrorSpy.mockRestore();
    });
    describe('handleAxiosError', () => {
      ...
      it('should handle non-axios errors', () => {
        ...
        expect(() => handleAxiosError(error, 'test')).toThrow(
          'Error inesperado al comunicarse con Directus',
        );
      });
    });
  });
