...
export default function GeneradorRecibos({ pago, venta, onGenerarPDF }: GeneradorRecibosProps) {
  const generarRecibo = () => {
    ...
    doc.text(`Fecha: ${pago.fecha_pago ? new Date(pago.fecha_pago).toLocaleDateString() : 'N/A'}`, 140, 55);
    ...
    doc.setFontSize(14);
    doc.text(`Monto: $${pago.monto.toLocaleString('es-MX', { minimumFractionDigits: 2 })} MXN`, 14, 85);
    
    doc.setFontSize(12);
    doc.text(`Concepto: Pago de Lote ${
    ...
    doc.text(`Forma de Pago: ${pago.metodo_pago ? pago.metodo_pago.toUpperCase() : 'N/A'}`, 14, 115);
    
    // Footer
    doc.setFontSize(10);
    doc.text('Este documento es un comprobante de pago válido.', pageWidth / 2, 140, { align: 'center' });
