...
const TablaComisiones: React.FC<TablaComisionesProps> = ({ 
  ...
  const comisionesFiltradas = useMemo(() => {
    return comisiones.filter(c => {
      ...
      const nombreVendedor = typeof c.vendedor_id === 'object' 
        ? `${c.vendedor_id.nombre} ${c.vendedor_id.apellido_paterno}`.toLowerCase() 
        : '';
      const matchVendedor = filtroVendedor ? nombreVendedor.includes(filtroVendedor.toLowerCase()) : true;
    ...
    <div className="bg-white shadow rounded-lg p-4 w-full">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              {['venta_id', 'vendedor_id', 'tipo_comision', 'monto_comision', 'estatus', 'fecha_pago_programada'].map((col) => (
                <th 
                  key={col}
                  scope="col" 
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={7} className="px-6 py-4 text-center text-gray-500">No se encontraron comisiones</td>
              </tr>
            ) : (
              comisionesPaginadas.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {typeof c.venta_id === 'object' ? String(c.venta_id.id).substring(0, 8) : String(c.venta_id).substring(0, 8)}...
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {typeof c.vendedor_id === 'object' ? `${c.vendedor_id.nombre} ${c.vendedor_id.apellido_paterno}` : 'N/A'}
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button onClick={() => onVerDetalles(c.id)} className="text-indigo-600 hover:text-indigo-900 mr-3">Ver</button>
                    {onMarcarPagada && c.estatus !== 'pagada' && (
                      <button onClick={() => onMarcarPagada(c.id)} className="text-green-600 hover:text-green-900">Pagar</button>
                    )}
                  </td>
                </tr>
              ))
      ...
      <div className="py-3 flex items-center justify-between border-t border-gray-200">
         ...
         <div className="flex-1 flex justify-between sm:hidden">
            <button onClick={() => setPaginaActual(p => Math.max(1, p - 1))} disabled={paginaActual === 1} className="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">Anterior</button>
            <button onClick={() => setPaginaActual(p => Math.min(totalPaginas, p + 1))} disabled={paginaActual === totalPaginas} className="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">Siguiente</button>
         </div>
         <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">Mostrando {(paginaActual - 1) * itemsPorPagina + 1} a {Math.min(paginaActual * itemsPorPagina, comisionesFiltradas.length)} de {comisionesFiltradas.length}</p>
            </div>
            <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
               {Array.from({ length: Math.min(5, totalPaginas) }, (_, i) => {
                 ...
                 return (
                  <button key={p} onClick={() => setPaginaActual(p)} className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${paginaActual === p ? 'z-10 bg-indigo-50 border-indigo-500 text-indigo-600' : 'bg-white border-gray-300 text-gray-500 hover:bg-gray-50'}`}>{p}</button>
                 );
               })}
            </nav>
         </div>
