...
const TablaVentas: React.FC<TablaVentasProps> = ({ 
  ...
  const [ordenColumna, setOrdenColumna] = useState<keyof Venta | 'cliente' | 'vendedor'>('fecha_venta');
  ...
  const ventasFiltradas = useMemo(() => {
    return ventas.filter(venta => {
      ...
      const nombreVendedor = typeof venta.vendedor_id === 'object' 
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              {['fecha_venta', 'cliente', 'vendedor', 'lote_id', 'monto_total', 'estatus'].map((col) => (
                <th 
                  key={col}
                  scope="col" 
              ...
              ventasPaginadas.map((venta) => (
                <tr key={venta.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {typeof venta.cliente_id === 'object' ? `${venta.cliente_id.nombre} ${venta.cliente_id.apellido_paterno}` : 'N/A'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {typeof venta.vendedor_id === 'object' ? `${venta.vendedor_id.nombre} ${venta.vendedor_id.apellido_paterno}` : 'N/A'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                      ${venta.estatus === 'liquidado' ? 'bg-green-100 text-green-800' : 
                        ...
                        venta.estatus === 'apartado' ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800'}`}>
                      {venta.estatus.toUpperCase()}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button onClick={() => onVerDetalles(venta.id)} className="text-indigo-600 hover:text-indigo-900 mr-3">Ver</button>
                    {onEditar && <button onClick={() => onEditar(venta.id)} className="text-blue-600 hover:text-blue-900 mr-3">Editar</button>}
                    {onGenerarContrato && venta.estatus === 'apartado' && (
                      <button onClick={() => onGenerarContrato(venta.id)} className="text-green-600 hover:text-green-900">Contrato</button>
                    )}
                  </td>
                </tr>
              ))
        ...
        <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-gray-700">
              Mostrando <span className="font-medium">{(paginaActual - 1) * itemsPorPagina + 1}</span> a <span className="font-medium">{Math.min(paginaActual * itemsPorPagina, ventasFiltradas.length)}</span> de <span className="font-medium">{ventasFiltradas.length}</span> resultados
            </p>
          </div>
          <div>
            <select
