...
const TablaPagos: React.FC<TablaPagosProps> = ({ 
  ...
  const pagosFiltrados = useMemo(() => {
    return pagos.filter(pago => {
      ...
      const ventaIdentificador = typeof pago.venta_id === 'object' 
    ...
    <div className="bg-white shadow rounded-lg p-4 w-full">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-4 text-center text-gray-500">No se encontraron pagos</td>
              </tr>
            ) : (
              pagosPaginados.map((pago) => (
                <tr key={pago.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {typeof pago.venta_id === 'object' ? String(pago.venta_id.id).substring(0, 8) : String(pago.venta_id).substring(0, 8)}...
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                      ${pago.estatus === 'pagado' ? 'bg-green-100 text-green-800' : 
                        pago.estatus === 'atrasado' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>
                      {pago.estatus.toUpperCase()}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button onClick={() => onVerDetalles(pago.id)} className="text-indigo-600 hover:text-indigo-900 mr-3">Ver</button>
                    {onMarcarPagado && pago.estatus !== 'pagado' && (
                      <button onClick={() => onMarcarPagado(pago.id)} className="text-green-600 hover:text-green-900 mr-3">Pagar</button>
                    )}
                    {onGenerarRecibo && pago.estatus === 'pagado' && (
                      <button onClick={() => onGenerarRecibo(pago.id)} className="text-gray-600 hover:text-gray-900">Recibo</button>
                    )}
                  </td>
                </tr>
              ))
      ...
      <div className="py-3 flex items-center justify-between border-t border-gray-200">
         ...
         <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">Mostrando {(paginaActual - 1) * itemsPorPagina + 1} a {Math.min(paginaActual * itemsPorPagina, pagosFiltrados.length)} de {pagosFiltrados.length}</p>
            </div>
