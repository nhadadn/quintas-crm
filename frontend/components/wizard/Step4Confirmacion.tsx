    ...
    <div className="max-w-4xl mx-auto bg-slate-800 p-8 rounded-xl shadow-lg border border-slate-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-slate-900 p-4 rounded-lg border border-slate-700">
          <h3 className="text-lg font-semibold text-emerald-400 mb-3 border-b border-slate-700 pb-2">Lote</h3>
          <div className="space-y-2 text-sm">
            <p><span className="text-slate-400">Número:</span> <span className="text-white font-medium">{loteSeleccionado.numero_lote}</span></p>
            <p><span className="text-slate-400">Zona:</span> <span className="text-white">{loteSeleccionado.zona}</span></p>
            <p><span className="text-slate-400">Manzana:</span> <span className="text-white">{loteSeleccionado.manzana}</span></p>
            <p><span className="text-slate-400">Área:</span> <span className="text-white">{loteSeleccionado.area_m2} m²</span></p>
            <p><span className="text-slate-400">Precio Lista:</span> <span className="text-white font-bold">${loteSeleccionado.precio_lista.toLocaleString('es-MX')}</span></p>
          </div>
        </div>

        {/* Resumen Cliente */}
        <div className="bg-slate-900 p-4 rounded-lg border border-slate-700">
          <h3 className="text-lg font-semibold text-emerald-400 mb-3 border-b border-slate-700 pb-2">Cliente</h3>
          <div className="space-y-2 text-sm">
            <p><span className="text-slate-400">Nombre:</span> <span className="text-white font-medium">{cliente.nombre} {cliente.apellido_paterno} {cliente.apellido_materno}</span></p>
            <p><span className="text-slate-400">RFC:</span> <span className="text-white">{cliente.rfc || 'N/A'}</span></p>
            <p><span className="text-slate-400">Email:</span> <span className="text-white">{cliente.email}</span></p>
            <p><span className="text-slate-400">Teléfono:</span> <span className="text-white">{cliente.telefono || 'N/A'}</span></p>
          </div>
        </div>

        {/* Resumen Términos */}
        <div className="bg-slate-900 p-4 rounded-lg border border-slate-700 md:col-span-2">
          <h3 className="text-lg font-semibold text-emerald-400 mb-3 border-b border-slate-700 pb-2">Términos Financieros</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="text-white font-bold">${terminos.monto_financiado.toLocaleString('es-MX')}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-white font-bold text-lg">${terminos.mensualidad?.toLocaleString('es-MX')}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-8 bg-slate-900 rounded-lg border border-slate-700 overflow-hidden">
        ...
        >
          <span className="font-semibold text-emerald-400">Tabla de Amortización Completa ({tablaAmortizacion.length} pagos)</span>
          <svg 
