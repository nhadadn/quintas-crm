          ...
          <tbody className="divide-y divide-slate-700">
            {tabla.map((fila) => (
              <tr key={fila.numero_pago} className="hover:bg-slate-700/50 transition-colors">
                ...
                <td className="px-6 py-4 text-right font-medium text-white">{formatCurrency(fila.cuota)}</td>
                <td className="px-6 py-4 text-right text-slate-300">{formatCurrency(fila.interes)}</td>
                <td className="px-6 py-4 text-right text-slate-300">{formatCurrency(fila.capital)}</td>
                <td className="px-6 py-4 text-right text-slate-300">{formatCurrency(fila.saldo_restante)}</td>
                <td className="px-6 py-4 flex justify-center">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${STATUS_STYLES[fila.estatus]}`}>
                    {fila.estatus}
                  </span>
                </td>
              </tr>
