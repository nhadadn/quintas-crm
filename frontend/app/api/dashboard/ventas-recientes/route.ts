...
export async function GET(request: Request) {
  ...
  try {
    const response = await axios.get(`${DIRECTUS_URL}/items/ventas`, {
      headers,
      params: {
        'fields': 'id,fecha_venta,monto_total,estatus,cliente_id.nombre,cliente_id.apellido_paterno,vendedor_id.nombre,vendedor_id.apellido_paterno,lote_id.numero_lote',
        'sort': '-fecha_venta',
        'limit': limit
      }
    });
