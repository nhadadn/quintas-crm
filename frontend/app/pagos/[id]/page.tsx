...
const getStatusClasses = (estatus: Pago['estatus'] | undefined | null): string => {
  const value = (estatus || '').toLowerCase();
  if (value === 'pagado') return 'bg-success/10 text-success border-success/40';
  if (value === 'atrasado' || value === 'vencido') return 'bg-danger/10 text-danger border-danger/40';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-3">
            ...
            <div className="sm:col-span-1">
              ...
              <dd className="mt-1 text-sm text-foreground">{getLoteIdentificador(pago.venta_id)}</dd>
            </div>
          </dl>
        </div>
      </div>
