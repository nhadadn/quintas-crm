...
export default function ReportesPage() {
  ...
  const handleExport = () => {
    ...
    window.open(
      `${process.env.NEXT_PUBLIC_API_URL}/pagos/reportes/ingresos?formato=pdf&fecha_inicio=${dateRange.start}&fecha_fin=${dateRange.end}`,
      '_blank',
    );
  };
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={metrics?.mrr_history || []}>
                ...
                <XAxis
                  ...
                  stroke="#94a3b8"
                  fontSize={12}
                  tickFormatter={(v) => format(new Date(v), 'MMM yyyy', { locale: es })}
                />
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={metrics?.revenue_forecast || []}>
                ...
                <XAxis
                  ...
                  stroke="#94a3b8"
                  fontSize={12}
                  tickFormatter={(v) => format(new Date(v), 'MMM', { locale: es })}
                />
                <YAxis stroke="#94a3b8" fontSize={12} tickFormatter={(v) => `$${v / 1000}k`} />
                <Tooltip
                  contentStyle={{
            ...
            <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
              <span className="text-2xl font-bold text-slate-700 dark:text-slate-200">
                {metrics?.subscriptions.total_active
                  ? (100 - (metrics.churn.churn_rate || 0)).toFixed(1)
                  : 0}
                %
              </span>
      ...
      {(subtext || trend !== undefined) && (
        <div className="flex items-center gap-2 text-sm">
          {trend !== undefined && trend !== 0 && (
            <span
              className={`flex items-center font-medium ${isGood ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}
            >
              {isPositive ? (
