export default (router, { services, exceptions, database, getSchema }) => {
  ...
  const calcularAmortizacion = (
    ...
    if (metodo === 'frances') {
      if (tasaMensual > 0) {
        cuotaFijaFrances =
          (monto_financiado * (tasaMensual * Math.pow(1 + tasaMensual, plazo_meses))) /
  ...
  router.get('/amortizacion/generar', async (req, res) => {
    try {
      const { venta_id, monto_total, enganche, plazo_meses, tasa_interes, fecha_inicio, metodo } =
        req.query;
      ...
      if (venta_id) {
        ...
        if (venta.estatus !== 'contrato') {
          ...
          throw new InvalidPayloadException(
            `La venta debe estar en estatus 'contrato' (Actual: ${venta.estatus})`
          );
        }
        ...
        if (!monto_total || !plazo_meses) {
          throw new InvalidPayloadException(
            'Faltan parámetros requeridos: monto_total y plazo_meses (o venta_id)'
          );
        }
      ...
      const tabla = calcularAmortizacion(montoFinanciado, plazo, tasa, fecha, metodo || 'frances');
  ...
  router.get('/simular-comisiones', async (req, res) => {
    try {
      ...
      const vendedor = await vendedoresService.readOne(vendedor_id, {
        fields: ['comision_porcentaje', 'comision_esquema'],
      });
  ...
  router.get('/:id', async (req, res) => {
    try {
      ...
      const venta = await ventasService.readOne(id, {
        fields: ['*', 'lote_id.*', 'cliente_id.*', 'vendedor_id.*', 'pagos.*', 'comisiones.*'],
      });
  ...
  router.post('/', async (req, res) => {
    ...
    try {
      ...
      if (lote.estatus !== 'disponible')
        throw new InvalidPayloadException(`El lote no está disponible (Estatus: ${lote.estatus})`);
      ...
      const vendedor = await vendedoresService.readOne(vendedor_id, {
        fields: ['id', 'activo', 'comision_porcentaje'],
      });
  ...
  router.delete('/:id', async (req, res) => {
    ...
    try {
      ...
      await trx('pagos')
        .where({ venta_id: id, estatus: 'pendiente' })
