...
export class WebhookService {
  ...
  async handlePaymentIntentSucceeded(paymentIntent, schema) {
    ...
    try {
      if (pago.venta_id) {
        // Aplicar el pago FIFO a la tabla de amortización
        await this._aplicarPagoAmortizacion(pago.venta_id, parseFloat(pago.monto || 0), pago.id);
...
// Utilidades internas
WebhookService.prototype._aplicarPagoAmortizacion = async function (ventaId, montoPago, pagoId) {
